...
int main()
{
    ...
    server.send_msg(server.create_msg<int>(100)); // send a message over bluetooth

    cout << "waiting for reply" << endl;
    int result = server.receive_msg().get_val<int>(); // wait for a reply message over bluetooth
