...
int main()
{
    ...
    // the way to send a message is you have to first create a message and specify its type
    // below, we see that we create a message of type int (because int is in the brackets)
    // and value 33 and then send it
    c.send_msg(c.create_msg<int>(33));
    this_thread::sleep_for(milliseconds(500)); // sleep half a second

    // Or, we can spread out the message creation and sending the message onto multiple lines
    MessageBuf m = c.create_msg<double>(3.141592); // create a message of type double (since double is in the brackets)
    ...
    c.send_msg(c.create_msg<char>(my_str_msg.c_str(), my_str_msg.size())); // to send a string, send its c_str and its size
    this_thread::sleep_for(milliseconds(500));
    ...
    // create the message (this time we're sending MyDummyClass, so put MyDummyClass in the brackets)
