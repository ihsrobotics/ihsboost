...
PosixQCommunicator::PosixQCommunicator(std::string name, size_t max_msgs)
    : FileCommunicator(), _name(name), max_msgs(max_msgs) {
    check_exists();
    open();
}

PosixQCommunicator::PosixQCommunicator(std::string name,
                                       size_t max_msgs,
                                       uint32_t max_msg_size)
    : FileCommunicator(max_msg_size), _name(name), max_msgs(max_msgs) {
    check_exists();
    open();
}

PosixQCommunicator::~PosixQCommunicator() { close(); }
...
MessageBuf PosixQCommunicator::receive_msg() {
    ...
    ssize_t ret = mq_receive(msg_q_id,
