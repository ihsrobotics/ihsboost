...
SHMCommunicator::SHMCommunicator(const char *path,
                                 int identifier,
                                 uint32_t max_msg_size)
    : SHMCommunicator(ftok(path, identifier), max_msg_size){};
SHMCommunicator::SHMCommunicator(int id, uint32_t max_msg_size)
    : FileCommunicator(max_msg_size), id(id) {
    check_exists();
    open();
}

SHMCommunicator::SHMCommunicator(const char *path, int identifier)
    : SHMCommunicator(ftok(path, identifier)){};
SHMCommunicator::SHMCommunicator(int id) : FileCommunicator(), id(id) {
    check_exists();
    open();
}

SHMCommunicator::~SHMCommunicator() { close(); }

bool SHMCommunicator::exists() {
    int ret = shmget(id,
                     MessageBuf::get_size(max_msg_size),
                     S_IRWXU | S_IRWXO | S_IRWXG | IPC_CREAT | IPC_EXCL);
...
void SHMCommunicator::open() {
    shm_id = shmget(id,
                    MessageBuf::get_size(max_msg_size),
                    S_IRWXU | S_IRWXO | S_IRWXG | IPC_CREAT);
