...
SocketServer::SocketServer(uint16_t port, uint32_t max_msg_size)
    : Communicator(max_msg_size), port(port) {
    open();
}
...
SocketClient::SocketClient(std::string ipv4_addr,
                           uint16_t port,
                           uint32_t max_msg_size)
    : Communicator(max_msg_size), port(port), ipv4_addr(ipv4_addr) {
    open();
}
SocketClient::SocketClient(std::string ipv4_addr, uint16_t port)
    : Communicator(), port(port), ipv4_addr(ipv4_addr) {
    open();
}
