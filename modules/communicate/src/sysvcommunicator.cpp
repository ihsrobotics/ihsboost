...
SysVCommunicator::SysVCommunicator(const char *path,
                                   int identifier,
                                   uint32_t max_msg_size)
    : SysVCommunicator(ftok(path, identifier), max_msg_size){};
SysVCommunicator::SysVCommunicator(int key, uint32_t max_msg_size)
    : FileCommunicator(max_msg_size), k(key) {
    check_exists();
    open();
}

SysVCommunicator::SysVCommunicator(const char *path, int identifier)
    : SysVCommunicator(ftok(path, identifier)){};
SysVCommunicator::SysVCommunicator(int key) : FileCommunicator(), k(key) {
    check_exists();
    open();
}

SysVCommunicator::~SysVCommunicator() { close(); }
...
void SysVCommunicator::send_bytes(char *bytes) {
    ...
    int ret = msgsnd(msg_q_id,
                     reinterpret_cast<const void *>(bytes),
                     MessageBuf::get_size(max_msg_size) - sizeof(uint32_t),
                     0);
    delete[] bytes;
...
MessageBuf SysVCommunicator::receive_msg() {
    ...
    // receive message (ignore the uint32_t because msgrcv deals with that
    // already)
    ssize_t ret = msgrcv(msg_q_id,
                         bytes,
                         MessageBuf::get_size(max_msg_size) - sizeof(uint32_t),
                         0,
                         0);
