...
bool send_values(Communicator *communicator, Container l) {
    ...
    communicator->send_msg(communicator->create_msg<T>(
        msg, static_cast<uint16_t>(boost::python::len(l))));
    delete[] msg;
    return true;
}
// send single values
template <typename T> bool send_val(Communicator *communicator, T val) {
...
template <typename T> T receive_val(Communicator *communicator) {
    MessageBuf m = communicator->receive_msg();
    return m.get_val<T>();
}
...
void export_communicate() {
    ...
    class_<SocketServer, bases<Communicator>>(
        "SocketServer",
        init<int, optional<int>>((arg("port"), arg("max_msg_size"))));
    class_<SocketClient, bases<Communicator>>(
        "SocketClient",
        init<string, int, optional<int>>((arg("ip"), arg("max_msg_size"))));
    class_<SHMCommunicator, bases<Communicator>>(
        "SHMCommunicator",
        init<int, optional<int>>((arg("id"), arg("max_msg_size"))));
    class_<PosixQCommunicator, bases<Communicator>>(
        "PosixQCommunicator",
        init<string, optional<int, int>>(
            (arg("name"), arg("max_msgs"), arg("max_msg_size"))));
    class_<SysVCommunicator, bases<Communicator>>(
        "SysVCommunicator",
        init<int, optional<int>>((arg("id"), arg("max_msg_size"))));
    class_<BluetoothClient, bases<Communicator>>(
        "BluetoothClient",
        init<string, optional<int>>(
            (arg("bluetooth_addr"), arg("max_msg_size"))));
    class_<BluetoothServer, bases<Communicator>>(
        "BluetoothServer", init<optional<int>>((arg("max_msg_size"))));
