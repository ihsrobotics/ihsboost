...
void turn_degrees(Subscriber *subscriber,
    ...
    THREE_PART_LOOP(max_speed,
                    min_speed,
                    accel_per_sec,
                    updates_per_sec,
                    MOVEMENT_FUNCTION(
                        static_cast<int>(accelerator.speed() * left_sign_val),
                        static_cast<int>(accelerator.speed() * right_sign_val)),
                    MOVEMENT_FUNCTION(
                        static_cast<int>(decelerator.speed() * left_sign_val),
                        static_cast<int>(decelerator.speed() * right_sign_val)),
                    degrees > 0 && subscriber->get_relative_angle() < degrees,
                    degrees < 0 && subscriber->get_relative_angle() > degrees,
                    subscriber->get_relative_angle())
...
void drive_straight(Subscriber *subscriber,
    ...
    while (!condition()) {
        if ((speed > 0 && subscriber->get_relative_angle() > 0) ||
            (speed < 0 && subscriber->get_relative_angle() < 0))
        {
            MOVEMENT_FUNCTION(static_cast<int>(speed * correction_proportion),
                              static_cast<int>(speed / correction_proportion));
        }
