...
// with the encoder functions
// --------------------------------------------------------------------------------------------------------

void rotate(double leftWheelSpeed,
            double rightWheelSpeed,
            double angle,
            double left_wheel_units,
...
double encoder_drive_straight(int speed,
    ...
    return (subscriber.get_relative_lenc_delta() * ENC_2_MM +
            subscriber.get_relative_renc_delta() * ENC_2_MM) /
           2.0;
}

double encoder_drive_straight(int max_speed,
                              double cm,
                              bool stop,
                              int min_speed,
                              double correction_proportion,
    ...
    THREE_PART_LOOP(
        ...
        accel_per_sec,
        updates_per_sec,
        if ((mm > 0 && subscriber.get_relative_lenc_delta() >
                           subscriber.get_relative_renc_delta()) ||
            (mm < 0 && subscriber.get_relative_lenc_delta() <
                           subscriber.get_relative_renc_delta())) {
            create_drive_direct(
                static_cast<int>(accelerator.speed() * correction_proportion),
                static_cast<int>(accelerator.speed() / correction_proportion));
        } else {
            create_drive_direct(
                static_cast<int>(accelerator.speed() / correction_proportion),
        ...
        if ((mm > 0 && subscriber.get_relative_lenc_delta() * ENC_2_MM < mm &&
            ...
            if ((mm > 0 && subscriber.get_relative_lenc_delta() >
                ...
                (mm < 0 && subscriber.get_relative_lenc_delta() <
                               subscriber.get_relative_renc_delta()))
            {
                create_drive_direct(static_cast<int>(decelerator.speed() *
                                                     correction_proportion),
                                    static_cast<int>(decelerator.speed() /
                                                     correction_proportion));
            }
            else {
                create_drive_direct(static_cast<int>(decelerator.speed() /
                    ...
                    subscriber.get_relative_lenc_delta() * ENC_2_MM < mm) ||
                   (mm < 0 &&
                    subscriber.get_relative_lenc_delta() * ENC_2_MM > mm)) {
            create_drive_direct(min_speed * sign_val, sign_val);
        } else { create_drive_direct(sign_val, min_speed * sign_val); },
        mm > 0 && subscriber.get_relative_distance() < mm,
        mm < 0 && subscriber.get_relative_distance() > mm,
        subscriber.get_relative_distance())
...
void encoder_drive_straight_pid(int speed,
                                double cm,
    ...
    LinearController accelerator(
        0, speed * sign_val, accel_per_sec, updates_per_sec);
    PIDController l_controller(proportional_coefficient,
                               integral_coefficient,
                               derivative_coefficient,
                               updates_per_sec);
    PIDController r_controller(proportional_coefficient,
                               integral_coefficient,
                               derivative_coefficient,
                               updates_per_sec);
           ...
           (mm < 0 && goal_delta > mm * MM_2_ENC / 2))
    {
        ...
        // starting at velocity of 0, then going to velocity of v linearly ->
        // forms a triangle so we integrate using triangle integration
        goal_delta =
            accelerator.speed() * static_cast<double>(updates) * dt / 2.0;

        if (subscriber.get_relative_lenc_delta() != lenc_delta_prev ||
            subscriber.get_relative_renc_delta() != renc_delta_prev)
        {
            // update internals
            ...
            create_drive_direct(static_cast<int>(process_speed(
                                    l_speed, accelerator.speed(), min_speed)),
                                static_cast<int>(process_speed(
                                    r_speed, accelerator.speed(), min_speed)));
            ...
            (mm < 0 && goal_delta > (mm - cached_distance) * MM_2_ENC)))
    {
        ...
        goal_delta = min(temp_goal_delta + (accelerator.speed() *
                                            static_cast<double>(updates) * dt),
                         (mm - cached_distance) * MM_2_ENC);
    ...
    updates = 0;
    LinearController decelerator(static_cast<int>(accelerator.speed()),
                                 min_speed * sign_val,
                                 accel_per_sec,
                                 updates_per_sec);
    while ((mm > 0 && (subscriber.get_relative_lenc_delta() < mm * MM_2_ENC ||
           ...
           ((mm < 0 && (subscriber.get_relative_lenc_delta() > mm * MM_2_ENC ||
                        subscriber.get_relative_renc_delta() > mm * MM_2_ENC))))
    {
        // trapezoidal integration from accelerator.speed() to
        // decelerator.speed() as b1 and b2 use (b1+b2)/2 * h, h is delta t
        goal_delta =
            min(temp_goal_delta + ((accelerator.speed() + decelerator.speed()) /
                                   2.0 * static_cast<double>(updates) * dt),
                mm * MM_2_ENC);
            ...
            subscriber.get_relative_renc_delta() != renc_delta_prev)
        {
            ...
            create_drive_direct(static_cast<int>(process_speed(
                                    l_speed, decelerator.speed(), min_speed)),
                                static_cast<int>(process_speed(
                                    r_speed, decelerator.speed(), min_speed)));
            lenc_delta_prev = subscriber.get_relative_lenc_delta();
            renc_delta_prev = subscriber.get_relative_renc_delta();
        }
...
void encoder_turn_degrees(int max_speed,
                          double degrees,
                          int min_speed,
                          double accel_per_sec,
                          int updates_per_sec) {
...
void encoder_turn_degrees(Speed turn_speed,
                          double degrees,
                          int updates_per_sec) {
    // initialize encoders
    EncoderSubscriber subscriber(updates_per_sec);
