...
/**
 * @brief Calibrate the ihsboost variables `min_gyro_val`, `max_gyro_val`, and
 * `mean_gyro_val` `raw_to_360_degrees` must be calibrated manually. Changes are
 * written to `bot-config.json` afterwards
 * @exception Json::RuntimeError is thrown if `bot-config.json` doesn't exist
 *
 * @param num_samples how many samples to take; this function takes samples at a
 * rate of 200 samples / sec
 * @param min_occurences how many times a value must occur in order for it to be
 * considered valid
 */
void calibrate_ihsboost_gyro(int num_samples = 1000,
                             uint16_t min_occurences = 8);

/**
 * @brief Drive straight using the gyroscope until told to stop
 *
 * @param speed the speed to drive at, can be positive or negative
 * @param stop_function the stop condition; it should return true when it is
 * time to stop
 ...
 */
void gyro_drive_straight(
    int speed,
    std::function<bool()> stop_function,
    bool stop = get_config().getBool("gyro_stop"),
 ...
 * defaults to 200
 */
void gyro_turn_degrees(
    Speed turn_speed,
    double degrees,
    int updates_per_sec = get_config().getInt("gyro_updates_per_sec"));

/**
 * @brief Turns the create, starting at rest and ending at rest, and turning at
 * max at max_speed
 *
 * @param max_speed the maximum speed to turn at, use a positive value
 * @param degrees how many degrees to turn (+ vals for CW, - vals for CCW)
 * @param min_speed the minimum speed to turn at, use a positive value, defaults
 * to 11
...
void gyro_turn_degrees(
    int max_speed,
    double degrees,
    int min_speed = get_config().getInt("gyro_min_speed"),
    double accel_per_sec = get_config().getDouble("gyro_accel_per_sec"),
    int updates_per_sec = get_config().getInt("gyro_updates_per_sec"));
