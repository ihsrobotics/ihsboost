...
/**
 * @brief Three part loop that deals with accelerating, decelerating, and any
 * extra driving
 *
 */
#define THREE_PART_LOOP(max_speed,                                             \
                        min_speed,                                             \
                        accel,                                                 \
                        updates,                                               \
    ...
    while ((cond_a / 2) || (cond_b / 2)) {                                     \
        ...
        if (accelerator.done()) {                                              \
            ...
            break;                                                             \
                                                                               ...
                                                                               \
    while (cached_amt != 0 &&                                                  \
           ((cond_a - cached_amt) || (cond_b - cached_amt))) {                 \
        accelerator_movement;                                                  \
                                                                               \
        msleep(accelerator.get_msleep_time());                                 \
    }                                                                          \
                                                                               \
    LinearController decelerator(                                              \
        static_cast<int>(accelerator.speed()), min_speed, accel, updates);     \
    while ((cond_a) || (cond_b)) {                                             \
        decelerator_movement;                                                  \
                                                                               \
        decelerator.step();                                                    \
 ...
 *
 * @param subscriber the subscriber that provides information on the current
 * angle
 ...
 */
void turn_degrees(Subscriber *subscriber,
                  int max_speed,
                  double degrees,
                  int min_speed,
...
/**
 * @brief Turn the given number of degrees sat the given speed using the given
 * subscriber
 *
 * @param subscriber the subscriber that provides information on the current
 * angle
 ...
 * @param condition the stop condition; it should return true when it is time to
 * stop
 * @param stop whether or not to stop the motors after finishing driving; if it
 * is false, the motors will continue spinning, if it is true, they will not
 * continue spinning
 ...
 * @param updates_per_sec how many updates to do per second
 */
void drive_straight(Subscriber *subscriber,
                    int speed,
