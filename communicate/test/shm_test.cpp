...
int main(int argc, const char *argv[])
{
    if (argc != 2)
    {
        cerr << "need to provide whether this is the sender or listener" << endl;
        cerr << "for intended results, this should only be called with 'sender' by a client" << endl;
        return 1;
    }
    ...
    if (string(argv[1]) == string("listener"))
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            assert_equals(flag_reader.receive_msg().get_val<bool>(), true, "getting ready flag");
            MessageBuf m = reader.receive_msg();

            // check that the sum of every element (excluding the last one) is equal to the final element
            int *back_ptr = m.get_ptr_val<int>() + m.get_length() - 1;
            assert_equals(accumulate(m.get_ptr_val<int>(), back_ptr - 1, 0), *back_ptr, "getting val");
        }
    }
    else if (string(argv[1]) == string("sender"))
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            sender.send_msg(sender.create_msg<int>(to_send.begin().base(), static_cast<uint16_t>(to_send.size())));
