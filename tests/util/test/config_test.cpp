...
void test_get_config() {
    ...
    assert_equals(get_config().getDouble("linear_accel"),
                  20000.0,
                  "getting overriden value");
    assert_equals(get_config().getInt("align_updates_per_sec"),
                  500,
...
void test_save_config() {
    ...
    assert_equals(20000.0,
                  conf.getDouble("linear_accel"),
                  "getting actual config values in save_config");
...
void test_config() {
    ...
    assert_equals(
        c.getInt("non_existent"), 0, "getting non-existent config value");
